...
capture program drop textfind4
program textfind4, rclass
      ...
      qui {
        ...
        ******************
        forv z = 1 / `n' {
          ...
          *** found
          gen ``i'keylen`z'' = cond(`key`z'', ustrlen(`"`y'"'), 0) if `touse'

          *** Fill in statistic 2
          gen ``i'`z'2' = ///
            cond(`key`z'', (ustrlen(`i') - ustrlen(ustrregexra(`i', `"`y'"', ///
            "", `case'))) / ``i'keylen`z'', ., .) if !mi(`i') & `touse'
          ...
          *** Run test for missing before filling variable in table
          capture assert ``i'`z'3' == .
            if _rc {
              sum ``i'`z'3' if !mi(``i'`z'3') & `touse', meanonly
          ...
          *** This variable only makes sense if the search is not exact
          if `"`exact'"' == "" {
            ...
            *** Run test for missing or null values before filling in table
            capture assert ``i'`z'4' == . | assert ``i'`z'4' == 0
              if _rc {
                sum ``i'`z'4' ///
                  if (!mi(``i'`z'4') | ``i'`z'4' != 0) & `touse', meanonly
          ...
          *** Run test for missing values before filling in table
          capture assert ``i'keytfidf`z'' == .
            if _rc {
              sum ``i'keytfidf`z'' if !mi(``i'keytfidf`z'') & `touse', meanonly
          ...
          *** Compare across means
          if `z' == 1 {
            ...
            *** Test whether the sample identified by keyword z + 1 is different
            *** from sample identified by z
        ...
        ***********************
        forv z = 1 / `m' {
          ...
          *** Fill in tempvar `notlength'
          gen ``i'notlen`z'' = cond(`not`z'', 0, ustrlen(`"`y'"'), .) if `touse'

          *** Fill in statistic 2
          gen ``i'`z'2' = ///
            cond(`not`z'' | ``i'notlen`z'' == ., ., ///
            (ustrlen(`i') - ustrlen(ustrregexra(`i', `"`y'"', "", `case'))) ///
             / ``i'notlen`z'') if !mi(`i') & `touse'
          ...
          *** Run test for missing values before filling in table
          capture assert ``i'nottfidf`z'' == .
            if _rc {
              sum ``i'nottfidf`z'' if !mi(``i'nottfidf`z'') & `touse', meanonly
        ...
        if `"`keyword'"' != "" {
          ...
          if `"`but'"' == "" {
            ...
            *** Run test for missing before filling variable in table
            capture assert ``i'len' == .
              if _rc {
                sum ``i'len' if !mi(``i'len') & `touse', meanonly
            ...
            if `n' == 1 {
              ...
              *** Run test for missing before filling variable in table
              capture assert ``i'`t'2' == .
                if _rc {
                  sum ``i'`t'2' if !mi(``i'`t'2') & `touse', meanonly
            ...
            ********************************
            if `"`exact'"' == "" & `n' == 1 {
              ...
              *** Run test for missing or null values before filling in table
              capture assert ``i'`t'4' == . | assert ``i'`t'4' == 0
                if _rc {
                  sum ``i'`t'4' ///
                    if (!mi(``i'`t'4') | ``i'`t'4' != 0) & `touse', meanonly
          ...
          ****************************
          if `m' == 1 {
            ...
            *** Fill in temporary variable
            gen ``i'totlen' = cond(`not', 0, ustrlen(`"`y'"'), .) ///
              if !mi(`i') & `touse'
      ...
      *************
      if `"`table'"' != "" {
        ...
        *** Table header
        ...
        di as text "Average finds per obs: the average occurence of " _continue
        ...
        di as text "Type-I error p-value:  the p-value for a means " _continue
        ...
        *** Filling out keywords by row
        if `"`keyword'"'!="" {
          ...
          *** Fill in table
          forv z = 1 / `n' {
            local y: word `z' of `keyword'
            di as text %-13s abbrev(`"`y'"', 12) _continue
        ...
        *** Fill in last row
        di as text _dup(80) "-"
        di as text %-13s abbrev("Total finds", 12) _continue
