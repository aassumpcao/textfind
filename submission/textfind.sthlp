...
document frequency (tf-idf) of the word used in the search; and the p-value of a
 means test between samples identified by different search criteria.
...
{it:"string2"}, ..., in each observation of {varname}, where {it:string} can be
...
{it:...}, in each observation of {varname}, where {it:string} can be text,
...
{phang}{cmd:exact} performs an exact search of {cmd:keyword()} in {varname} and
...
the word length of {varname} for which the search criteria is
found.
...
{phang}{cmd:. use "https://github.com/aassumpcao/textfind/blob/master/CivilServantsNeverland.dta?raw=true", clear}{p_end}
...
{pstd}
Here is the result using {cmd:textfind} (table preamble omitted). It identifies
the same observations as the commands above but it does so in one line of code
and returning six statistics on the quality of match.
...
found in {varname}.
{p_end}
{synopt:{cmd:r(max)}} maximum number of words in largest string in
{varname}.
{p_end}
